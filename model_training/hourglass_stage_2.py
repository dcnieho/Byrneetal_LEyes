...
dataset = 'Chugh'   # Chugh or EDS2020
...
IMAGE_SIZE, N_CRs, segment_pipeline, _, image_pipeline = simulations.get_hourglass_pipeline(dataset=dataset, stage=2)
...
"""## 2. Set up training, Transfer training"""
batch_size = 8
num_epochs = 20 if isChugh else 500
lr = 1e-5
scheduler_gamma = 0.9
earlystop_tolerance = 5 if isChugh else 30
earlystop_mindelta = 0

model = torch.load(f'{dataset}_stage_1.pt').to(device)
...
if isChugh:
    ...
    weight, alpha, gamma = 1, 0.25, 20
criterion = training.BCElogitsWithDiceFocalLoss(weight, alpha=alpha, gamma=gamma, reduction="mean")
earlystopper = training.EarlyStopping(patience=earlystop_tolerance, verbose=False, delta=earlystop_mindelta, path=f'{dataset}_stage_2.pt')
